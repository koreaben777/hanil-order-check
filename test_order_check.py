...
def test_rules_file():
    with tempfile.TemporaryDirectory() as d:
        ...
        oc.set_rule("P1", p, width_plus=200, items=[" 2pd2040nt2n "], grades=["a1"], note="엠보 무관")
        r = oc.rule_for("P1", oc.load_rules(p))
        assert (r["width_plus"], r["items"], r["grades"], r["note"]) == (200, ["2PD2040NT2N"], ["A1"], "엠보 무관")
        assert oc.rule_for("P2", oc.load_rules(p))["width_plus"] == 100                       # 다른 거래처는 기본
        oc.set_rule("P1", p, note="폭만")                                                      # 부분 갱신, 나머지 유지
        assert oc.rule_for("P1", oc.load_rules(p))["width_plus"] == 200
...
def test_check_with_fake_db():
    ...
    so = pd.DataFrame([("2PD2040NT1N", 1070, 2000, "A", "SOZ1", 1, "20260901", "X", 856.0, 342.4, 513.6, 10),   # 10롤 중 6롤 미출하
                      ...
                      columns=so_cols)
    prq = pd.DataFrame(columns=["NO_PRQ", "DT_DLV", "QT_PRQ", "qt_wo", "QT_WORK", "NO_SO"])
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "rules.json"
            # 기본 기준(폭 +100): 재고 24 + 슬리팅 5 + 생산 19 = 50% · 10% · 40%
            ...
            assert (r["stock_rolls"], r["sub_rolls"], r["prod_rolls"]) == (24, 5, 19)
            assert r["decisions"] == ["재고출하", "대체검토", "생산의뢰"]
            assert list(r["subs"].kind) == ["폭+30"] and list(r["subs"].alloc) == [5]
            assert "재고출하 24롤 (50%)" in oc.report(r) and "생산의뢰 19롤 (40%)" in oc.report(r)
            # 거래처 기준 추가(폭 +200 · 품목 NT2N · 등급 A1): 슬리팅 5+9, 등급 4, 품목 0(미출하) → 생산 3
            oc.set_rule("P1", p, width_plus=200, items=["2PD2040NT2N"], grades=["A1"], note="엠보 무관")
            r = oc.check(oc.Order("2PD2040NT1N", 1070, 2000, "A", rolls=48, partner="P1"), today=date(2026, 9, 10), rules_file=p)
            assert list(r["subs"].kind) == ["폭+30", "폭+180", "등급", "품목"]
            assert list(r["subs"].alloc) == [5, 9, 4, 0] and (r["sub_rolls"], r["prod_rolls"]) == (18, 6)
