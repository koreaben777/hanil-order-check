...
- CP-SAT 회로·작업 인터벌·NoOverlap. 정수 가중치 사전식 **긴급 가중 납기 지연 → 전환시간(가열 포함)+CR 환산손실 → 완료 시각** 최소화(잉여 하한 제약 `완료 시각 ≥ 총 가공시간 + 선택된 전환시간 합` 포함). 동률 해소용 상승 횟수 항은 없다(증명이 끝나지 않아 삭제). 결과의 `proven_optimal`이 참이면 시간 제한 안에 하한과 만나 이 모델·계수 기준 최적이 증명된 것이고, 거짓이면 `gap_changeover_minutes`가 전환 손실 하한과의 차이(분)다. 색상 묶음은 연성 선호이며 긴급 납기로 깨질 수 있다. 솔버 설정은 시간 제한뿐이다(1단 부분문제별, 2단 전체).
- `transition_rules.json`에서 전환값을 변경한다. `[추정] S-05` 평량 하강 Δ는 `0.0375 × max(0, Δ−30)`시간, `[추정] S-04/S-06` 평량 상승은 가열 비용 `min(0.005 × 상승 g, 0.5h)`(상한은 한 번의 상승이 색상 전환보다 비싸지지 않게 하는 보호), 농업용 조건 변경은 1h다. 전환 손실 합계에는 가열 시간이 포함되며 `changeover_components_h`에 색상·하강·상승·농업용 성분이 따로 있다. 같은 색·직전 100g에서 **100→60→30: 0.375h**, 직행 **100→30: 1.5h**. C-02 블랙→화이트는 5.5h·CR 6롤이다. 다른 진→연은 5.5h 추정, CR은 BK→WH에만 붙인다. 알 수 없는 색상 계열이 낀 전환은 보수적으로 5.5h로 잡고 경고를 낸다.
- 전환값은 명시한 `rules_file` → 모듈 옆 `transition_rules.json` → 내장 기본값 순으로 선택한다. 명시한 파일이 없거나 잘못된 경우는 오류이며 조용히 기본값으로 대체하지 않는다. JSON 과 내장 기본값은 같은 내용이어야 한다(테스트가 대조).
